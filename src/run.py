...
#   parser.add_argument('-o', '--output-format', help='出力形式', default='text', choices=['text', 'xml'])
    ...
    parser_title.add_argument('image-id', help='画像ID（yyyyMMddHHmmss）')
    ...
    parser_delete.add_argument('image-id', help='画像ID（yyyyMMddHHmmss）')
    parser_delete.set_defaults(handler=command_set_title)
    # get
    parser_get = sub.add_parser('get', help='取得する。`get -h`')
    parser_get.add_argument('image-id', help='画像ID（yyyyMMddHHmmss）')
